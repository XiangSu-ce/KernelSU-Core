...
fn set_identity(uid: u32, gid: u32, groups: &[u32]) -> Result<()> {
    ...
    .context("set_thread_groups failed")?;
    let gid = Gid::from_raw(gid);
    let uid = Uid::from_raw(uid);
    set_thread_res_gid(gid, gid, gid).context("set_thread_res_gid failed")?;
    set_thread_res_uid(uid, uid, uid).context("set_thread_res_uid failed")?;
    Ok(())
}
...
pub fn root_shell() -> Result<()> {
    ...
    let mut pw_gid_for_user: Option<u32> = None;
    if free_idx < matches.free.len() {
        let name = &matches.free[free_idx];
        let (resolved_uid, resolved_gid) = {
            let pw = CString::new(name.as_str())
                .ok()
                .and_then(|c_name| unsafe { libc::getpwnam(c_name.as_ptr()).as_ref() });

            if let Some(pw) = pw {
                (pw.pw_uid, Some(pw.pw_gid))
            } else {
                (
                    name.parse::<u32>()
                        .map_err(|_| anyhow!("Invalid user: {name}"))?,
                    None,
                )
            }
        };
        uid = resolved_uid;
        pw_gid_for_user = resolved_gid;
        free_idx += 1;
    }

    // if there is no gid provided, use user's primary gid if available; otherwise use uid.
    let gid = gid.unwrap_or_else(|| pw_gid_for_user.unwrap_or(uid));

    // If no -c is provided and additional arguments are present after user,
    // pass them to the invoked shell/program.
    if cmd_args.is_empty() && free_idx < matches.free.len() {
        cmd_args = matches.free[free_idx..].to_vec();
    }
    ...
    command = command.args(cmd_args).arg0(arg0);
    Err(command.exec().into())
}
