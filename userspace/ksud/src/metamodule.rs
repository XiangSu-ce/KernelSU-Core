    ...
    let _ = crate::module::foreach_module(All, |module_path| {
            ...
            && is_metamodule(&props)
        {
            if result.is_none() {
                ...
                warn!(
                    "Multiple metamodules detected, ignoring extra candidate: {}",
                    module_path.display()
                );
            }
        }
...
pub fn ensure_symlink(module_path: &Path) -> Result<()> {
    ...
    if symlink_path.exists() || symlink_path.is_symlink() {
        ...
        if symlink_path.is_symlink() {
            ...
            std::fs::remove_file(symlink_path).with_context(|| "Failed to remove old file")?;
        } else {
...
pub fn exec_metauninstall_script(module_id: &str) -> Result<()> {
    ...
    let result =
        Command::new(assets::BUSYBOX_PATH)
            ...
            .current_dir(metauninstall_path.parent().with_context(|| {
                format!("Invalid script path: {}", metauninstall_path.display())
            })?)
