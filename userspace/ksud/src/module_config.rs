...
pub fn load_config(module_id: &str, config_type: ConfigType) -> Result<HashMap<String, String>> {
    ...
    for i in 0..count {
        ...
        validate_config_key(&key)
            .with_context(|| format!("Invalid key format for entry {i}: '{key}'"))?;
        ...
        validate_config_value(&value)
            .with_context(|| format!("Invalid value for entry {i}, key '{key}'"))?;
...
pub fn get_all_module_configs() -> Result<HashMap<String, HashMap<String, String>>> {
    ...
    let mut entries: Vec<_> = fs::read_dir(config_root)
