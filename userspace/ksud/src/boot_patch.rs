...
mod android {
    ...
    fn parse_kmi_from_modules() -> Result<String> {
        use std::io::BufRead;
        // find a *.ko in /vendor/lib/modules (deterministic order)
        let mut candidates: Vec<_> = std::fs::read_dir("/vendor/lib/modules")?
            .flatten()
...
fn validate_output_name(name: &str) -> Result<()> {
    ...
    ensure!(!name.is_empty(), "output name cannot be empty");
    ensure!(
        path.file_name().is_some_and(|f| f == name),
        "output name must be a plain filename"
    );
    ensure!(name != "." && name != "..", "invalid output name");
    Ok(())
}
...
fn parse_kmi_from_boot(magiskboot: &Path, image: &PathBuf, workdir: &Path) -> Result<String> {
    ...
    let extracted_kernel = workdir.join("kernel");
    if extracted_kernel.exists() {
        parse_kmi_from_kernel(&extracted_kernel, workdir)
    } else {
        // Fallback for formats where magiskboot doesn't emit "kernel" directly.
