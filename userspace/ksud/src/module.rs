...
pub fn foreach_module(
    ...
    if !modules_dir.exists() {
        info!(
            "module directory {} does not exist, skip",
            modules_dir.display()
        );
        return Ok(());
    }
    let mut module_dirs: Vec<_> = std::fs::read_dir(modules_dir)?
        .flatten()
...
pub fn exec_script<T: AsRef<Path>>(path: T, wait: bool) -> Result<()> {
    ...
    let script_dir = path
        .parent()
        .with_context(|| format!("script path has no parent: {}", path.display()))?;
    ...
    result.map_err(|e| anyhow!("Failed to exec {}: {e}", path.display()))
}
...
pub fn prune_modules() -> Result<()> {
    foreach_module(All, |module| {
        ...
        let module_id = module.file_name().and_then(|n| n.to_str());
        ...
        } else if let Some(id) = module_id {
            if let Err(e) = metamodule::exec_metauninstall_script(id) {
                warn!("Failed to exec metamodule uninstall for {id}: {e}",);
            }
        } else {
            warn!(
                "Skip metamodule uninstall callback for non-utf8 module path: {}",
                module.display()
            );
        }
        ...
        if let Some(id) = module_id {
            if let Err(e) = crate::module_config::clear_module_configs(id) {
                warn!("Failed to clear configs for {id}: {e}");
            }
        } else {
            warn!(
                "Skip config cleanup for non-utf8 module path: {}",
                module.display()
            );
        }
    ...
    let remaining_modules: Vec<_> = match std::fs::read_dir(defs::MODULE_DIR) {
        Ok(dir) => dir
...
fn mark_all_modules(flag_file: &str) -> Result<()> {
    ...
    if !modules_dir.exists() {
        info!(
            "{} not exists, skip mark_all_modules",
            modules_dir.display()
        );
        return Ok(());
    }
