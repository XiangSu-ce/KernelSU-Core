...
fn parse_bracket_objs(input: &str) -> IResult<&str, SeObject<'_>> {
    let (input, (_, words, _)) = (
        tag("{"),
        take_while1(|c: char| is_sepolicy_char(c) || c.is_whitespace()),
        tag("}"),
    )
        .parse(input)?;
...
fn parse_nonspace_token(input: &str) -> IResult<&str, &str> {
...
impl<'a> SeObjectParser<'a> for GenFsCon<'a> {
        ...
        Self: Sized,
    {
        ...
        let (input, _) = space1(input)?;
        let (input, path) = parse_nonspace_token(input)?;
        let (input, _) = space1(input)?;
        let (input, context) = parse_nonspace_token(input)?;
