...
static int do_get_allow_list(void __user *arg)
{
    ...
    bool success = ksu_get_allow_list((int *)cmd.uids, (int *)&cmd.count,
                                      ARRAY_SIZE(cmd.uids), true);
...
static int do_get_deny_list(void __user *arg)
{
    ...
    bool success =
        ksu_get_allow_list((int *)cmd.uids, (int *)&cmd.count,
                           ARRAY_SIZE(cmd.uids), false);
...
static void clear_try_umount_list(void)
{
...
static int add_try_umount(void __user *arg)
{
    ...
    case KSU_UMOUNT_ADD: {
        ...
        len = strncpy_from_user(buf, (const char __user *)cmd.arg, 256);
        if (len <= 0)
            return -EFAULT;
        if (len >= sizeof(buf))
            return -ENAMETOOLONG;
    ...
    case KSU_UMOUNT_DEL: {
        ...
        len = strncpy_from_user(buf, (const char __user *)cmd.arg,
                                sizeof(buf));
        if (len <= 0)
            return -EFAULT;
        if (len >= sizeof(buf))
...
int ksu_install_fd(void)
{
    ...
    filp = anon_inode_getfile("[timerfd]", &anon_ksu_fops, NULL,
                              O_RDWR | O_CLOEXEC);
    if (IS_ERR(filp)) {
