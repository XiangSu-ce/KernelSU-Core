        ...
        if ((namelen == 8) && (strncmp(name, "base.apk", namelen) == 0)) {
            ...
            if (is_manager) {
                ...
                *my_ctx->stop = 1;
                ...
                if (!apk_data) {
                    pr_err("Failed to allocate apk_path_hash\n");
                    return FILLDIR_ACTOR_CONTINUE;
                }
                apk_data->hash = hash;
...
void track_throne(bool prune_only)
{
    ...
    for (;;) {
        ...
        line_len = pos - line_start - 1;
        if (line_len <= 0) {
            line_start = pos;
            continue;
        }
        if (line_len >= sizeof(buf)) {
            pr_warn("track_throne: skip overlong packages.list line (%lld)\n",
                    (long long)line_len);
            line_start = pos;
            continue;
        }
        ...
        read_pos = line_start;
        count = kernel_read(fp, buf, line_len, &read_pos);
        if (count != line_len) {
            pr_err("track_throne: short read (%zd/%lld)\n", count,
                   (long long)line_len);
            break;
        }
        buf[line_len] = '\0';
