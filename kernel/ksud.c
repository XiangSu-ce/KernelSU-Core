...
static const char *ksu_get_obf_path(void)
{
    ...
    static DEFINE_MUTEX(obf_path_lock);

    if (READ_ONCE(ready))
        return decoded;

    mutex_lock(&obf_path_lock);
    if (!ready) {
        OBF_INIT_RUNTIME(ksud_path);
        decoded = OBF_GET(ksud_path);
        WRITE_ONCE(ready, true);
    }
    mutex_unlock(&obf_path_lock);
    return decoded;
}
...
const char *ksu_get_init_rc(void)
{
    static char rc[512];
    static bool built;
    static DEFINE_MUTEX(init_rc_lock);

    if (!READ_ONCE(built)) {
        mutex_lock(&init_rc_lock);
        if (built) {
            mutex_unlock(&init_rc_lock);
            return rc;
        }
                             ...
                             struct user_arg_ptr *envp, int *flags)
{
    ...
    static atomic_t init_second_stage_executed = ATOMIC_INIT(0);
    ...
    if (unlikely(strcmp(filename->name, system_bin_init) == 0 && argv)) {
        char buf[16];
        if (atomic_read(&init_second_stage_executed) == 0 &&
            check_argv(*argv, 1, "second_stage", buf, sizeof(buf)) &&
            atomic_cmpxchg(&init_second_stage_executed, 0, 1) == 0) {
    ...
    if (unlikely(
            ...
            strcmp(filename->name, app_process) == 0 &&
            argv)) {
        char buf[16];
        if (check_argv(*argv, 1, "-Xzygote", buf, sizeof(buf)) &&
            atomic_cmpxchg(&first_zygote, 1, 0) == 1) {
            pr_info("exec zygote, /data prepared, second_stage: %d\n",
                    atomic_read(&init_second_stage_executed));
            rcu_read_lock();
            struct task_struct *init_task =
                rcu_dereference(current->real_parent);
            if (init_task) {
                if (task_work_add(init_task, &on_post_fs_data_cb, TWA_RESUME))
                    pr_warn("on_post_fs_data task_work_add failed\n");
            }
            rcu_read_unlock();
            stop_execve_hook();
        }
...
/*
 * fops_proxy / orig_read / ksu_rc_pos / ksu_rc_str / ksu_rc_len:
 * These static variables are guarded by the atomic_cmpxchg on rc_hooked
 * (see ksu_handle_sys_read). Only one thread ever wins the race, so once
 * set they are effectively read-only from the read-proxy path.
 */
...
{
    static atomic_t safe_mode = ATOMIC_INIT(0);
    if (atomic_read(&safe_mode)) {
