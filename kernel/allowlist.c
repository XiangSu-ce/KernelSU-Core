...
bool ksu_get_app_profile(struct app_profile *profile)
{
    ...
    mutex_lock(&allowlist_mutex);
    has_key = profile->key[0] != '\0';
    list_for_each (pos, &allow_list) {
        p = list_entry(pos, struct perm_data, list);
        bool uid_match = profile->current_uid == p->profile.current_uid;
        bool key_match = !has_key || !strcmp(profile->key, p->profile.key);
        if (uid_match && key_match) {
...
out:
    if (profile->current_uid <= BITMAP_UID_MAX) {
        ...
        if (profile->allow_su) {
            ...
            /* Check if UID is already in the array to avoid duplicates */
            for (k = 0; k < allow_list_pointer; k++) {
                if (allow_list_arr[k] == profile->current_uid) {
                    already_in_arr = true;
                    break;
                }
            }
            if (!already_in_arr) {
                if (allow_list_pointer >= ARRAY_SIZE(allow_list_arr)) {
                    ...
                    WARN_ON(1);
                    /* Undo list insertion for new nodes to keep state consistent */
                    if (!result) {
                        list_del(&p->list);
                        kfree(p);
                    }
...
void ksu_get_root_profile(uid_t uid, struct root_profile *out_profile)
{
    struct perm_data *p = NULL;
    struct list_head *pos = NULL;
    ...
    list_for_each (pos, &allow_list) {
        ...
        if (uid == p->profile.current_uid && p->profile.allow_su) {
            if (!p->profile.rp_config.use_default) {
                memcpy(out_profile, &p->profile.rp_config.profile,
                       sizeof(*out_profile));
                mutex_unlock(&allowlist_mutex);
                return;
            }
    ...
    // use default profile
    memcpy(out_profile, &default_root_profile, sizeof(*out_profile));
    mutex_unlock(&allowlist_mutex);
}

bool ksu_get_allow_list(int *array, int *length, size_t capacity, bool allow)
{
    struct perm_data *p = NULL;
    struct list_head *pos = NULL;
    int i = 0;
    ...
    *length = (i < capacity) ? i : capacity;

    if (truncated) {
        pr_warn("allow list truncated: total=%d capacity=%zu allow=%d\n", i,
                capacity, allow);
    }
    mutex_unlock(&allowlist_mutex);
...
static void do_persistent_allow_list(struct callback_head *_cb)
{
    ...
    list_for_each (pos, &allow_list) {
        ...
        wret = kernel_write(fp, &p->profile, sizeof(p->profile), &off);
        if (wret != sizeof(p->profile)) {
            pr_err("save_allow_list write entry failed: %zd\n", wret);
            break;
        }
    }
...
void ksu_load_allow_list()
{
    ...
    while (true) {
        ...
        if (ret != sizeof(profile)) {
            pr_warn("load_allow_list short read: %zd/%zu, stop parsing\n", ret,
                    sizeof(profile));
            break;
        }
