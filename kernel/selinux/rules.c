...
void apply_kernelsu_rules()
{
    ...
    /* Suppress execve/audit noise for KSU domain only */
    ksu_dontaudit(db, KERNEL_SU_DOMAIN, ALL, "process", "transition");
    ksu_dontaudit(db, KERNEL_SU_DOMAIN, ALL, "process", "execmem");
                      ...
                      char **object)
{
    ...
    n = strncpy_from_user(buf, user_object, buf_sz);
    if (n <= 0) {
        return -EINVAL;
    }
    if (n >= buf_sz) {
...
static int copy_user_sepol_str(char *dst, size_t dst_sz, char __user *src,
                               const char *name)
{
    long n;
    ...
    if (n <= 0) {
        pr_err("sepol: copy %s failed.\n", name);
        return -EINVAL;
    }
    if (n >= dst_sz) {
        dst[dst_sz - 1] = '\0';
        pr_err("sepol: %s too long.\n", name);
        return -ENAMETOOLONG;
    }
...
int handle_sepolicy(unsigned long arg3, void __user *arg4)
{
    ...
    if (cmd == CMD_NORMAL_PERM) {
        ...
        if (copy_user_sepol_str(operation, sizeof(operation), data.sepol4,
                                "operation") < 0) {
            goto exit;
        }
        if (copy_user_sepol_str(perm_set, sizeof(perm_set), data.sepol5,
                                "perm_set") < 0) {
            goto exit;
        }
        ...
        if (copy_user_sepol_str(src, sizeof(src), data.sepol1, "src") < 0) {
            goto exit;
        }
        ...
        if (copy_user_sepol_str(type, sizeof(type), data.sepol1, "type") < 0) {
            goto exit;
        }
        if (copy_user_sepol_str(attr, sizeof(attr), data.sepol2, "attr") < 0) {
            goto exit;
        }
        ...
        if (copy_user_sepol_str(attr, sizeof(attr), data.sepol1, "attr") < 0) {
            goto exit;
        }
        ...
        if (copy_user_sepol_str(tgt, sizeof(tgt), data.sepol2, "tgt") < 0) {
            goto exit;
        }
        if (copy_user_sepol_str(cls, sizeof(cls), data.sepol3, "cls") < 0) {
            goto exit;
        }
        if (copy_user_sepol_str(default_type, sizeof(default_type), data.sepol4,
                                "default_type") < 0) {
            goto exit;
        }
        char *real_object;
        if (data.sepol5 == NULL) {
            ...
            if (copy_user_sepol_str(object, sizeof(object), data.sepol5,
                                    "object") < 0) {
                goto exit;
            }
            real_object = object;
        ...
        if (copy_user_sepol_str(name, sizeof(name), data.sepol1, "name") < 0) {
            goto exit;
        }
        if (copy_user_sepol_str(path, sizeof(path), data.sepol2, "path") < 0) {
            goto exit;
        }
        if (copy_user_sepol_str(context, sizeof(context), data.sepol3,
                                "context") < 0) {
            goto exit;
        }
