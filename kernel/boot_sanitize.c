...
static void scrub_legacy_ringbuf(void)
{
	...
	addr = ksu_lookup_name("log_buf_len");
	if (!addr)
		return;
	p_log_buf_len = (u32 *)addr;
	log_buf_len = *p_log_buf_len;

	addr = ksu_lookup_name("log_first_idx");
	if (!addr)
		return;
	p_log_first_idx = (u32 *)addr;

	addr = ksu_lookup_name("log_next_idx");
	if (!addr)
		return;
...
static void scrub_modern_ringbuf(void)
{
	 ...
	 */
	/* Optional probe: some kernels expose 'prb', some don't.
	 * We intentionally do not gate scrubbing on this symbol. */
	addr = ksu_lookup_name("prb");
	if (addr && !*(void **)addr)
		return;
