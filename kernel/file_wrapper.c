...
static struct file *ksu_unwrap_file(struct file *f)
{
    struct ksu_file_wrapper *data;

    if (!f || !f->f_op || f->f_op->release != ksu_wrapper_release)
        return f;
...
static int ksu_wrapper_open(struct inode *ino, struct file *fp)
{
    ...
    if (!new_fops) {
        ...
        ksu_release_file_wrapper(wrapper);
        fput(orig_file);
        return -ENOENT;
    }
    replace_fops(fp, new_fops);
    /* ksu_create_file_wrapper() took its own reference to orig_file. */
    fput(orig_file);
    return 0;
}
...
static ssize_t ksu_wrapper_read_iter(struct kiocb *iocb, struct iov_iter *iovi)
{
    ...
    struct file *orig = data->orig;
    struct file *saved = iocb->ki_filp;
    ssize_t ret;
    iocb->ki_filp = orig;
    ret = orig->f_op->read_iter(iocb, iovi);
    iocb->ki_filp = saved;
    return ret;
}
...
static ssize_t ksu_wrapper_write_iter(struct kiocb *iocb, struct iov_iter *iovi)
{
    ...
    ssize_t ret;
    iocb->ki_filp = orig;
    ret = orig->f_op->write_iter(iocb, iovi);
    iocb->ki_filp = saved;
    return ret;
}
                              ...
                              unsigned int v)
{
    ...
    struct file *orig = data->orig;
    struct file *saved = kiocb->ki_filp;
    int ret;
    kiocb->ki_filp = orig;
    ret = orig->f_op->iopoll(kiocb, icb, v);
    kiocb->ki_filp = saved;
    return ret;
}
#else
static int ksu_wrapper_iopoll(struct kiocb *kiocb, bool spin)
{
    ...
    int ret;
    kiocb->ki_filp = orig;
    ret = orig->f_op->iopoll(kiocb, spin);
    kiocb->ki_filp = saved;
    return ret;
}
#endif
                                           ...
                                           unsigned int flags)
{
    ...
    return orig_out->f_op->copy_file_range(orig_in, pos_in, orig_out, pos_out,
                                       len,
                                       flags);
}
                                           ...
                                           unsigned int remap_flags)
{
    ...
    return orig_out->f_op->remap_file_range(orig_in, pos_in, orig_out, pos_out,
                                            len, remap_flags);
}
...
int ksu_install_file_wrapper(int fd)
{
    ...
    struct file *wrapper_file = ksu_anon_inode_create_getfile_compat(
        "[anon_inode]", &file_wrapper_data->ops, file_wrapper_data,
...
{
    /* anon_inode_mnt is borrowed from kernel, nothing to release */
}
