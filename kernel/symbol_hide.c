...
static atomic_t module_list_hidden = ATOMIC_INIT(0);
 ...
 * We use the centralized ksu_lookup_name() to resolve it.
 */
 ...
 */
static bool hide_module_from_list(void)
{
	if (!p_module_mutex) {
		pr_warn("symbol_hide: module_mutex not resolved, cannot safely hide module\n");
		return false;
	}
	if (atomic_cmpxchg(&module_list_hidden, 0, 1) != 0)
		return false;
...
static int symbol_hide_set(u64 value)
{
	...
	if (value && !atomic_read(&module_list_hidden)) {
		if (hide_module_from_list())
