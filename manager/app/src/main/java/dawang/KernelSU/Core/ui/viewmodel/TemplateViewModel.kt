...
import dawang.KernelSU.Core.profile.Capabilities
import dawang.KernelSU.Core.profile.Groups
...
class TemplateViewModel : ViewModel() {
    ...
    val templateList by derivedStateOf {
        ...
        templates.sortedWith(comparator)
    }
    ...
    suspend fun fetchTemplates(sync: Boolean = false) {
        ...
        val refreshedTemplates = withContext(Dispatchers.IO) {
            ...
            listAppProfileTemplates().mapNotNull(::getTemplateInfoById)
        }
        withContext(Dispatchers.Main) {
    ...
    return runCatching {
        ...
        val templateInfo = TemplateViewModel.TemplateInfo(
            ...
            namespace = Natives.Profile.Namespace.valueOf(
                namespace.uppercase(Locale.ROOT)
            ).ordinal,
