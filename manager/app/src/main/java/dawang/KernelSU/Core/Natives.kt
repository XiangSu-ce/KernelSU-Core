...
object Natives {
    ...
    /**
     * Generic feature API — query/toggle any kernel feature by ID.
     */
    external fun isFeatureEnabled(featureId: Int): Boolean
    external fun setFeatureEnabled(featureId: Int, enabled: Boolean): Boolean
    external fun isFeatureSupported(featureId: Int): Boolean

    // Feature IDs — must match kernel/feature.h
    const val FEATURE_PROP_SPOOF = 2
    const val FEATURE_PROC_HIDE = 3
    const val FEATURE_DEBUG_DISABLE = 4
    const val FEATURE_LOG_SILENT = 5
    const val FEATURE_SYMBOL_HIDE = 6
    const val FEATURE_MOUNT_SANITIZE = 7
    const val FEATURE_STEALTH_FILTER_IO = 8
    const val FEATURE_STEALTH_MODLOADER = 9
    const val FEATURE_STEALTH_EXEC = 10
    const val FEATURE_STEALTH_FILEIO = 11
    const val FEATURE_STEALTH_IPC = 12
