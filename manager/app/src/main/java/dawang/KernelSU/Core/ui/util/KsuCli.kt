...
import dawang.KernelSU.Core.BuildConfig
...
suspend fun getFeaturePersistValue(feature: String): Long? = withContext(Dispatchers.IO) {
    val shell = getRootShell()
    val out = shell.newJob()
        .add("${getKsuDaemonPath()} feature get --config ${shellQuote(feature)}")
        .to(ArrayList<String>(), null)
        .exec().out
...
fun install() {
    ...
    val result = execKsud("install --magiskboot ${shellQuote(magiskboot)}", true)
...
fun flashModule(
    ...
    val input = resolver.openInputStream(uri)
        ?: return FlashResult(-1, "Failed to open module uri: $uri", false)

    input.use {
        val file = File.createTempFile("module-", ".zip", ksuApp.cacheDir)
...
fun runModuleAction(
    ...
    val result = shell.newJob().add("${getKsuDaemonPath()} module action ${shellQuote(moduleId)}")
...
fun restoreBoot(
    ...
    val result = flashWithIO(
        "${getKsuDaemonPath()} boot-restore -f --magiskboot ${shellQuote(magiskboot.absolutePath)}",
        onStdout,
        onStderr
    )
    return FlashResult(result)
...
fun uninstallPermanently(
    ...
    val magiskboot = File(ksuApp.applicationInfo.nativeLibraryDir, "libmagiskboot.so")
    val result = flashWithIO(
        "${getKsuDaemonPath()} uninstall --magiskboot ${shellQuote(magiskboot.absolutePath)}",
        onStdout,
        onStderr
    )
    return FlashResult(result)
...
fun installBoot(
    ...
    val bootFile = bootUri?.let { uri ->
        val input = resolver.openInputStream(uri) ?: return FlashResult(
            -1,
            "Failed to open boot image uri: $uri",
            false
        )
        input.use {
            val bootFile = File.createTempFile("boot-", ".img", ksuApp.cacheDir)
    ...
    val magiskboot = File(ksuApp.applicationInfo.nativeLibraryDir, "libmagiskboot.so")
    var cmd = "boot-patch --magiskboot ${shellQuote(magiskboot.absolutePath)}"
    ...
    when (lkm) {
        is LkmSelection.LkmUri -> {
            val input = resolver.openInputStream(lkm.uri) ?: return FlashResult(
                -1,
                "Failed to open lkm uri: ${lkm.uri}",
                false
            )
            lkmFile = input.use {
                val file = File.createTempFile("kernelsu-lkm-", ".ko", ksuApp.cacheDir)
                file.outputStream().use { output ->
                    it.copyTo(output)
                }
    ...
    val result = try {
        flashWithIO("${getKsuDaemonPath()} $cmd", onStdout, onStderr)
    } finally {
        bootFile?.delete()
        lkmFile?.delete()
    }
...
fun reboot(reason: String = "") {
    ...
    if (reason.isBlank()) {
        ShellUtils.fastCmd(shell, "/system/bin/svc power reboot || /system/bin/reboot")
    } else {
        val safeReason = shellQuote(reason)
        ShellUtils.fastCmd(
            shell,
            "/system/bin/svc power reboot $safeReason || /system/bin/reboot $safeReason"
        )
    }
}
...
suspend fun isAbDevice(): Boolean = withContext(Dispatchers.IO) {
    ...
    when (ShellUtils.fastCmd(shell, "${getKsuDaemonPath()} $cmd").trim().lowercase(Locale.ROOT)) {
...
fun isSepolicyValid(rules: String?): Boolean {
    ...
    val shell = getRootShell()
    val result =
        shell.newJob().add("${getKsuDaemonPath()} sepolicy check ${shellQuote(rules)}")
            .to(ArrayList(), null)
            .exec()
    return result.isSuccess
}
...
fun getSepolicy(pkg: String): String {
    ...
    val result =
        shell.newJob()
            .add("${getKsuDaemonPath()} profile get-sepolicy ${shellQuote(pkg)}")
            .to(ArrayList(), null)
            .exec()
...
fun setSepolicy(pkg: String, rules: String): Boolean {
    ...
    val result = shell.newJob()
        .add(
            "${getKsuDaemonPath()} profile set-sepolicy ${shellQuote(pkg)} ${
                shellQuote(
                    rules
                )
            }"
...
fun setAppProfileTemplate(id: String, template: String): Boolean {
    val shell = getRootShell()
    val cmd =
        "${getKsuDaemonPath()} profile set-template ${shellQuote(id)} ${shellQuote(template)}"
...
fun forceStopApp(packageName: String) {
    val shell = getRootShell()
    val result = shell.newJob().add("am force-stop ${shellQuote(packageName)}").exec()
