package dawang.KernelSU.Core.ui.webui
...
import dawang.KernelSU.Core.ui.util.createRootShell
import dawang.KernelSU.Core.ui.util.listModules
import dawang.KernelSU.Core.ui.util.withNewRootShell
...
class WebViewInterface(private val state: WebUIState) {
    ...
    private val maxPackageQueryChars = 256 * 1024
    private val maxPackageQueryItems = 4096
    private fun shellQuote(arg: String): String {
        return "'" + arg.replace("'", "'\"'\"'") + "'"
    }
    private fun isValidCallbackName(callback: String): Boolean {
        return callback.matches(Regex("[A-Za-z_$][A-Za-z0-9_$]*(\\.[A-Za-z_$][A-Za-z0-9_$]*)*"))
    }
    ...
    private fun processOptions(sb: StringBuilder, options: String?) {
        val opts = try {
            if (options == null) JSONObject() else JSONObject(options)
    ...
    fun exec(
        ...
        if (!isValidCallbackName(callbackFunc)) {
            return
        }
    ...
    fun spawn(command: String, args: String, options: String?, callbackFunc: String) {
        ...
        if (!TextUtils.isEmpty(args)) {
            finalCommand.append(shellQuote(command)).append(" ")
            try {
                ...
                // Fallback: treat args as a single plain argument.
                finalCommand.append(shellQuote(args)).append(" ")
            }
        } else {
            finalCommand.append(shellQuote(command))
        }
        ...
        completableFuture.thenAccept { result ->
            ...
            state.webView?.post {
                state.webView?.loadUrl(emitExitCode)
            }
            ...
            if (result.code != 0) {
                ...
                state.webView?.post {
                    state.webView?.loadUrl(emitErrCode)
                }
            }
        }.whenComplete { _, _ ->
    ...
    fun toast(msg: String) {
        state.webView?.post {
            val view = state.webView ?: return@post
    ...
    fun fullScreen(enable: Boolean) {
        val context = state.webView?.context ?: return
    ...
    fun getPackagesInfo(packageNamesJson: String): String {
        if (packageNamesJson.length > maxPackageQueryChars) {
            return JSONArray().toString()
        }
        val packageNames = try {
            JSONArray(packageNamesJson)
        } catch (_: JSONException) {
            return JSONArray().toString()
        }
        val jsonArray = JSONArray()
        val appMap = SuperUserViewModel.apps.associateBy { it.packageName }
        val count = minOf(packageNames.length(), maxPackageQueryItems)
        for (i in 0 until count) {
            val pkgName = try {
                packageNames.getString(i)
            } catch (_: JSONException) {
                continue
            }
